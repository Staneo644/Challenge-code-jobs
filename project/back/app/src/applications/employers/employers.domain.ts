...
export class EmployersDomain implements IEmployersDomain {
  ...
  async createEmployer(employerData: createEmployer): Promise<boolean> {
    ...
    const enterprise = await this.enterprisesDomain.getEnterpriseById(
      Number(employerData.enterprise_id),
    );
    if (enterprise) {
      ...
      if (ret !== null)
        this.enterprisesDomain.updateEnterprise(enterprise.id, {
          employers: [...enterprise.employers, ret],
        });
      return ret !== null;
    }
    return null;
