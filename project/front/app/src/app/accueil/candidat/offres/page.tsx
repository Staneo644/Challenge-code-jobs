  ...
  return (
    <button
      className={`w-10 h-10 rounded-full flex items-center justify-center text-white ${
        buttonClasses[type ? "accept" : "reject"]
      }`}
      onClick={onClick}
    >
      {type === true ? "✔" : "✘"}
...
export default function Home() {
  ...
  const showNextJob = async (valid:boolean) => {
    ...
    const ret = await addSeeingJob(email, jobs[currentJobIndex].id, valid);
  ...
  return (
    <div className="h-full w-full bg-gray-100">
      <main className="flex flex-col h-screen">
        <Template>
          <div className="bg-white p-4 rounded-lg shadow-md h-[calc(85vh-148px)]  relative">
            {" "}
            {/* Ajout de la classe relative */}
            <div className="mb-4 max-h-[calc(85vh-238px)] overflow-auto">
              ...
              {jobs && !noJobs && (
                <div className="flex items-center justify-center flex-col">
                  ...
                  <Image
                    src={jobs[currentJobIndex].imageBuffer.toString()}
                    width={500}
                    height={500}
                    alt="job image"
                    className="max-h-70 center object-center group-hover:opacity-75 flex"
                  />
                  <div className="text-2xl font-bold text-gray-800 text-center p-4">
            ...
            <div className="absolute bottom-4 left-0 right-4 text-center flex justify-between w-full">
              {" "}
              {/* Ajustez la classe CSS ici */}.
              <button
